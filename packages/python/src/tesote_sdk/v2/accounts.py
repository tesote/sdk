"""v2 accounts resource. Adds ``sync``."""
...
class AccountsResource:
    """`/v2/accounts` -- list, get implemented; sync stubbed."""
    ...
    def list(self, *, cache_ttl: Optional[float] = None) -> List[Dict[str, Any]]:
        response = self._transport.request("GET", self._PREFIX, cache_ttl=cache_ttl)
        body = response.json
    ...
    def sync(self, account_id: str, *, idempotency_key: Optional[str] = None) -> Dict[str, Any]:
        raise NotImplementedError("v2 accounts.sync not yet implemented")
